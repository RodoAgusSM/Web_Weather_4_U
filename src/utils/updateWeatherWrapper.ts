...
const formatTimeByLanguage = (weatherData: Weather): Weather => {
    ...
    return {
        ...
        lastTimeChecked: isEnglish
            ? getLastTimeChecked12HoursFormat()
            : getLastTimeChecked(),
        lastDateChecked: isEnglish
            ? getLastDateCheckedAmerican()
            : getLastDateChecked(),
    };
};
...
const convertMeasurementUnits = (weatherData: Weather, targetUnit: string): Weather => {
    ...
    return {
        ...
        windSpeed: isImperial
            ? kmToMiles(weatherData.windSpeed)
            : milesToKm(weatherData.windSpeed),
        visibility: isImperial
            ? kmToMiles(weatherData.visibility)
            : milesToKm(weatherData.visibility)
    };
};
